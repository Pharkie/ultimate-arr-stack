...
Started with a bug report from someone deploying the stack on a UGREEN NAS, whose Gluetun would not start and who had worked around it with `privileged: true`. The cap that was missing turned out to be breaking a second thing on every VPN type, and pulling that thread found three more checks in the same shape as 1.8.0 and 1.9.0: **they ran, they reported success, and they could not have observed the failure they existed to catch.**

### Added
- **`scripts/check-vpn.sh` now runs every 5 minutes from cron and pings an Uptime Kuma push monitor on success only**, so Kuma raises the alarm when the pings *stop*. The inversion matters: an HTTP monitor asks "is the service answering", which a leaking VPN answers perfectly well, while a push monitor also catches the case no HTTP monitor can — the check not running at all. `detect-vpn-zombies.sh` was fine and would have caught this release's namespace breakage; nothing invoked it at the moment it mattered. Bound to the existing default notification, because a monitor that alerts nobody is a dashboard nobody opens, and listed in the pre-commit expected monitors, because a monitor whose job is to notice silence fails silently when it is the thing that goes missing.
- **An explicit DNS probe in `check-vpn.sh`**, covering what Gluetun's health check deliberately stopped watching (below). One probe covers all four tunneled services, since they share Gluetun's namespace and therefore its resolver. It runs before the egress checks: those all resolve a hostname, so with DNS down they fail together and the report would otherwise blame egress for a name-resolution fault.
- **Test coverage harvested from `feat/harvest-e2e-coverage`** — unit tests for `detect-vpn-zombies.sh`, which had none; a live Gluetun-namespace check in the e2e suite; assertions that the *arr apps can still reach what they depend on; and shellcheck over the repo's own shell scripts.

### Fixed
- **Gluetun's `cap_drop: ALL` was taking away CHOWN and DAC_OVERRIDE, which it needs.** Adding only `NET_ADMIN` is what every Gluetun guide says, and it is correct — but only for people who left Docker's default capability set in place. Two consequences. `VPN_TYPE=openvpn` could never start, dying on `chown /etc/openvpn/target.ovpn: operation not permitted` and retrying forever, while `.env.example` documents `OPENVPN_USER`/`OPENVPN_PASSWORD` and so advertised a path that could not work. And on **every** VPN type, including the wireguard default this stack ships, Gluetun could not write `/tmp/gluetun/ip`: the running container had 347 permission-denied errors and `/v1/publicip/ip` had been returning an empty string the whole time. Reproduced all three cases against v3.41 with throwaway containers before fixing: NET_ADMIN alone gives the reported chown error, `+CHOWN` moves the failure to `auth.conf`, `+CHOWN +DAC_OVERRIDE` reaches the VPN handshake.
- **A Pi-hole wobble could tear down the VPN and never recover.** `DNS_ADDRESS=172.20.0.5` disables Gluetun's internal resolver, and its health check resolved `github.com`/`cloudflare.com` *by name* — through Pi-hole. A Pi-hole stall was therefore indistinguishable from a dead tunnel, and Gluetun's answer to a failed health check is to rebuild the tunnel: a remedy that cannot fix DNS, applied in a loop, taking every VPN-side container's network down on each pass. On 2026-08-05 that produced **323 VPN rebuilds and 10 container restarts** between 19:32 and 21:18, ending only when the container was restarted by hand. Pi-hole was not down for that hour and three quarters — its FTL query log, which survives container restarts and was the only witness left, shows it flapping, with multi-minute gaps that Gluetun's restart clusters land inside. `HEALTH_TARGET_ADDRESSES` now pins the check to `1.1.1.1:443` and `8.8.8.8:443` so it tests the tunnel and nothing else. Verified by staging the outage rather than reasoning about it: with DNS black-holed inside Gluetun's namespace for 3m30s against a 1-minute health check interval, it stayed healthy with zero restarts.
- **`check-vpn.sh` reported success when it could not measure a service at all.** A tunneled service whose egress came back empty printed `WARN`, was skipped, and did not affect the exit code — so the run in which all four had lost their network still ended on "OK: every tunneled service egresses through Gluetun", exit 0. The exit code is the entire product here: the script's own header advertises cron use, where nobody reads the output and a 0 is silence. Unmeasurable is now a failure. This is the same error the 1.8.0 rewrite fixed in the comparison itself, spelled `continue` instead of `==`.
- **Seerr's server ids are probed rather than assumed** — the check had a hardcoded `/0`.

### Changed
- **Image bump** — `sabnzbd` 5.1.1 → 5.1.2. Patch release surfaced by the pre-commit image check; config volume backed up before the recreate regardless.
- **`docs/REFERENCE.md` now states that a container must never be pointed at a `.lan` name.** Those all resolve to Traefik's macvlan address, which is deliberately unreachable from the container bridge on the same host, so every `.lan` URL fails from inside any container — IPv4 forced, whichever name. Found because Sonarr's Home Assistant webhook was set to `homeassistant.lan:8123` and had been failing silently for days; Radarr already pointed straight at the device and its real port. It bites hardest in webhook fields, where nothing validates the URL and the symptom arrives days later as a health check about "notifications" that mentions neither DNS nor macvlan — and the same name works perfectly in a browser, which is on the LAN where macvlan is reachable.
- **`docs/UTILITIES.md` documents the UGOS crontab trap.** `crontab <file>` cannot write its temp file into `/var/spool/cron/` as the admin user, and writing `/var/spool/cron/crontabs/<user>` directly *looks* like it works — the file is user-owned, the write succeeds, `crontab -l` lists the entry — but cron never reloads it and the job silently never runs. A one-minute canary entry proves it in two minutes; assuming otherwise would have left a monitoring setup that monitored nothing. Install needs `sudo crontab -u <user> <file>`.
