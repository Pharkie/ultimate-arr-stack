...
- **`configure-apps.sh`'s Bazarr language handling, rewritten after a review found it wrong in three ways.** (1) The profile step only created a profile when the list was *empty* and the default-profile step hard-coded id 1, so any Bazarr with a different layout — profile 1 not English, or profile 1 deleted and recreated as 2, which is what Bazarr's UI always does — had its defaults pointed at a wrong or nonexistent profile and reported ✓. (2) The languages step pruned *every* profile's languages down to English, so a user's French profile became one that searched for nothing. (3) The profile was created *after* Sonarr/Radarr were connected, and Bazarr stamps the default profile onto series and movies as it first inserts them — so on a fresh install the initial sync left the whole library without a profile. Now: the decision lives in `scripts/lib/bazarr-language-plan.py` (unit-tested against fresh, French-only, English-with-Latvian and adopt-by-contents states) — the profile named `English` is managed, else one whose languages are exactly the wanted set is adopted, else one is created with the next free id; both defaults point at that id; other profiles are never touched; Languages Filter ticks become the wanted languages plus whatever other profiles use. And it runs before the connections step.
- **`_api_request` reported ✓ for writes that never reached the server.** It returned the HTTP status as its exit code, and bash reads curl's `000` — what the write-out prints when the connection failed — as 0. Every `if api_post …; then ok` in the script was green on connection-refused. Transport failures now return 1, timeouts 2, and the HTTP code is never the exit status.
- **Every request is bounded, at the shared layer.** `_api_request` and `bazarr_settings_post` share one curl capture with `--connect-timeout 5` and `--max-time` (`API_TIMEOUT`, default 60 s; `BAZARR_POST_TIMEOUT` for Bazarr writes; `BAZARR_SCAN_TIMEOUT`, default 600 s, for the one Bazarr write that rescans the whole library inside the request). The previous fix bounded only Bazarr's wrapper, only decoded the timeout at one of six call sites, and told the user "the settings were written" on a code that also fires when the connect itself stalled. The helper now prints the diagnosis itself, hedged, and every call site keeps the plain `if` form. The values are validated (`90s` used to make curl print a usage error and the step report a generic failure; `0` silently disabled the bound).
- **No more `docker restart bazarr` after the Bazarr section.** Bazarr applies settings inside the POST and never restarts its process for them (read from v1.6.0: the only restarts on that path are its Sonarr/Radarr SignalR clients). The bounce was redundant, cost the ~40 s healthcheck window on every fresh run, and after the timeout change could fire into a request Bazarr was still executing.
