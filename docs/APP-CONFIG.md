...
3. **Languages — do this before connecting Sonarr/Radarr:** Settings → Languages:
   ...
   > **Why first:** Bazarr stamps the default profile onto each series and movie when it first learns of them, and connecting Sonarr/Radarr starts that sync immediately. Set the profile afterwards and your existing library has none — only titles added later get it, and you're left fixing the rest with Mass Edit. `configure-apps.sh` runs these in the same order: it manages the profile named **English** (or adopts one whose languages are exactly English, whatever it's called), creates it with the next free id if there isn't one, points both defaults at that id, and leaves any other profiles alone.
